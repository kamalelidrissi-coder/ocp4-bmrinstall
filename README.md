...
1. Download [config files](https://github.com/kamalelidrissi-coder/ocp4-bmrinstall) for each of the services

   ```bash
   git clone https://github.com/kamalelidrissi-coder/ocp4-bmrinstall
   ```
...
1. Set a Static IP for OCP network interface `nmtui-edit ens37` 
...
1. Copy the install-config.yaml included in the clones repository to the install directory
   ...
   ```bash
   cp ~/ocp4-bmrinstall/install-config.yaml ~/ocp-install
   ```
